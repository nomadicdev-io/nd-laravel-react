	...
	@if(!empty($old_file_name) && File::exists(storage_path($path.$old_file_name)))
		<div class="uploadPreview img_uploaded">
			<div class="upImgWrapper">
				...
				@if($type=="image")
				<img src="{{ getFilePath('programmes/'.$old_file_name) }}" class="uploadPreview"/>
					@else
					<img src="{{ asset('assets/admin/img/file.png') }}" class="uploadPreview"/>
					<a target="_blank" href="{{ apa('download-general-file/'.$old_file_name) }}" class="uploadPreview"> View/Download</a>
				@endif
			</div>
...
<div class = "uploadControlWrapper input_parent">
	<input type = "file" class = "form-control singleuploader" data-slug="{{ $slug }}" data-allowed="{{ (implode(",", $mimes)) }}" data-type="{{ $type }}" title = "Select File" id = "{{ $control_name.'_file' }}" name = "{{$control_name.'_file' }}" {{ (($required==true && empty($old_file_name))? " required ": '' ) }} />
	<div class = "choose">
