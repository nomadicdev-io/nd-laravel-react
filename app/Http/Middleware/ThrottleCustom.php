...
class ThrottleCustom
{
    ...
    public function handle($request, Closure $next, $maxAttempts = 4, $decayMinutes = 1)
    {
        ...
        if ($this->limiter->tooManyAttempts($key, $maxAttempts)) {
			...
			if ((starts_with(request()->path(), 'api')) || request()->ajax() ) {
    ...
    protected function buildJsonResponse($key, $maxAttempts)
    {
		$response = [
			'status'=>false,
			'response'=>trans('messages.too_many_attempts_wait_for_few_mins'), 
			'userMessage'=>trans('messages.too_many_attempts_wait_for_few_mins')
		]; 
